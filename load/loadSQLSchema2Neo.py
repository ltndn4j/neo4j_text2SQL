...
def load_schema(db_conn: psycopg2.extensions.connection, driver: neo4j.GraphDatabase.driver, schema_name: str):
    print(f"Loading schema '{schema_name}' to Neo4j database....")
...
def load(driver: neo4j.GraphDatabase.driver, initialize: bool = False):
    ...
    print("Loading SQL schema to Neo4j database....")
    ...
    if initialize or input("Do you want to erase all the data from the Neo4j database before loading (y/n) ? ") == "y":
        with driver.session() as session:
            session.run("MATCH (n) DETACH DELETE n")
            print("Neo4j database reseted successfully.")
